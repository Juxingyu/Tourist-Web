    ...
    <script type="text/javascript">
        ...
        function sendComment() {
            ...
            $.ajax({
                ...
                data: {'commentContent': comcontent, 'id': id},    //参数值
                type: "post",  //请求方式
                success: function (data) {
                    ...
                    if (data === "0") {
                        ...
                        window.location.href
                            = "${pageContext.request.contextPath }/front/food/foodInfo?id=" + id;
                    }
                },
            });
        }
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data === "0") {
                        alert("收藏成功");
                        window.location.href
                            = "${pageContext.request.contextPath }/front/food/foodInfo?sceId=" + sceId;
                    }
                },
            });
        }
...
<div class="bread_box" style="width: 1150px;">
    ...
    <span> &gt;&gt;</span>
    <%--<a href="${pageContext.request.contextPath }/front/scenery/sceType?scenictypeId=${requestScope.scenictype.scenictypeId}">${requestScope.scenictype.scenictpyeName}</a>
    <span> &gt;&gt;</span>--%>
    <a href="#" style="color: #0c7cb5">${requestScope.food.foodName}</a>
</div>
<div class="prosum_box" style="width: 1150px;">
    <%--图片展示--%>
    <div class="prosum_left">
        <img src="${pageContext.request.contextPath }/static/images/${requestScope.food.pic}" alt="">
    </div>
    <div class="prosum_right">
        <p class="pros_title" style="border-bottom: 4px solid #046DB6">${requestScope.food.foodName}</p>
        <div class="pros_other">
            <div id="secDesc" style="display: inline-block;
                ...
                *zoom: 1;
                ...
                /*white-space: nowrap;*/
                "><span style="color: #8c8c8c">美食信息：</span>${requestScope.food.foodDesc}</div>
            <br><br>

            <p>参考价格 ：${requestScope.food.price}</p>
            <p>地址 ：${requestScope.food.address}</p>
        </div>
        ...
        <%-- <div class="pros_price">
             <p class="collect">
                 &lt;%&ndash;为收藏过&ndash;%&gt;
                 <c:choose>
                     <c:when test="${requestScope.collectByUIdAndSceId}">&lt;%&ndash;已经收藏&ndash;%&gt;
...
<div class="you_need_konw" style="width: 1150px">
    ...
    <div class="notice">
        ...
        <div class="row">
            <div class="col-md-9">
                <c:choose>
                    <c:when test="${empty requestScope.foodComment}">
                        <h3>暂无评论</h3>
                    </c:when>
                    <c:otherwise>
                        <ul class="media-list">
                            <c:forEach items="${requestScope.foodComment}" var="food">
                                <c:forEach items="${requestScope.usersCom}" var="users">
                                    <li class="media">
                                        ...
                                        <div class="media-body">
                                            <h4 class="media-heading">${users.petName}</h4>

                                            <p>${food.commentContent}</p>

                                            <div class="ds-comment-footer">
                                    <span class="ds-time" datetime="${food.updateDate}"
                                          title="2016年1月28日 下午8:13:29"><fmt:formatDate value="${food.updateDate}"
                ...
                <%--写评论--%>
                <c:choose>
                    <c:when test="${empty sessionScope.users.petName}">
