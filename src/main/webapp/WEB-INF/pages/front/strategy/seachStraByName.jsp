...
<div class="page_one">
    <div class="contant">
        <div class="crumbs">
            ...
            <p>旅游攻略><span>搜索结果</span></p>
        </div>
        <div class="xinxi clearfix">
            <!--搜索结果展示-->
            <div class="left">
                <c:choose>
                    <c:when test="${empty requestScope.straList}">
                        <h1 style="font-size: 30px;text-align: center;line-height: 460px"><strong>未检测到与输入相关内容</strong>
                        </h1>
                    </c:when>
                    <c:otherwise>
                        <ul>
                            <c:forEach items="${requestScope.straList}" var="stra">
                                <li>
                                    <div class="img">
                                        <img src="${pageContext.request.contextPath }/static/images/${stra.straPic}"
                                             alt="${stra.straId}" width="300px" height="170px"></div>
                                    <div class="text1">
                                        <p>【${stra.straTitle}】</p>
                                        <br/>
                                        <p>${stra.straContext}</p>
                                    </div>
                                    <div class="price">
                                        <p class="price_num" style="font-size: 14px">
                                    <span>
                                        <fmt:formatDate value="${stra.updateDate}"
                                                        pattern="yyyy-MM-dd"/>
                                    </span>
                                        </p>
                                        <p>
                                            <a href="${pageContext.request.contextPath }/front/strategy/straInfo?straId=${stra.straId}">查看详情</a>
                                        </p>
                                    </div>
                                </li>
                            </c:forEach>
                ...
                <c:if test="${requestScope.page.pages>1}">
                    ...
                    <div class="pageNum">
                        <ul>
                            <li><a href="${pageContext.request.contextPath }/front/strategy/seachByName?pageNo=1">首页</a>
                            </li>
                            <c:choose>
                                <c:when test="${requestScope.page.hasPreviousPage}">
                                    <li class="threeword"><a
                                            href="${pageContext.request.contextPath }/front/strategy/seachByName?pageNo=${page.prePage}">上一页</a>
                                    </li>
                                </c:when>
                                <c:otherwise>
                            ...
                            <c:forEach begin="1" end="${requestScope.page.pages}" var="pageSize">
                                <c:choose>
                                    <c:when test="${pageSize==page.pageNum}">
                                        <li>
                                            <a href="${pageContext.request.contextPath }/front/strategy/seachByName?pageNo=${pageSize}&pageSize=6"
                                               style="background: #C30D23 none repeat scroll 0% 0%;color:#ffffff;">${pageSize}</a>
                                        </li>
                                    </c:when>
                                    <c:otherwise>
                                        <li>
                                            <a href="${pageContext.request.contextPath }/front/strategy/seachByName?pageNo=${pageSize}&pageSize=6">${pageSize}</a>
                                        </li>
                                    </c:otherwise>
                                </c:choose>
                            </c:forEach>
                            <c:choose>
                                <c:when test="${requestScope.page.hasNextPage}">
                                    <li class="threeword"><a
                                            href="${pageContext.request.contextPath }/front/strategy/seachByName?pageNo=${page.nextPage}">下一页</a>
                                    </li>
                                </c:when>
                                <c:otherwise>
                                    <li class="threeword" style="pointer-events:none;background-color:#F7F7F6">
                            ...
                            <li class="threeword">
                                <a href="${pageContext.request.contextPath }/front/strategy/seachByName?pageNo=${page.pages}">末页</a>
                            </li>
                        </ul>
                    </div>
                </c:if>
            ...
            <div class="right">
                ...
                <ul>
                    <c:forEach items="${requestScope.straRefer}" var="stra" begin="1" end="5">
                        <li>
                            <a href="javascript:window.parent.location.href='${pageContext.request.contextPath  }/front/strategy/straInfo?straId=${stra.straId}'">
                                ...
                                <div class="right">
                                    ...
                                    <p style="font-size: 10px">收藏<span
                                            style="font-size: 10px">共<span>${stra.praseCount}</span>次</span>
                                    </p>
                                </div>
                            </a>
                        </li>
