...
<body>
<div class="row" style="height: 4150px">
            ...
            <div class="ui segment">
                <div id="data_table_wrapper" class="ui grid dataTables_wrapper">
                    <table id="data_table"
                        ...
                        <c:forEach items="${requestScope.strategyList }" var="aa" varStatus="statu">
                            <tr>
                                <td>${statu.count}</td>
                                <td style="font-size: 12px">${aa.straTitle }</td>
                                <td><img src="${pageContext.request.contextPath }/static/images/${aa.straPic}"
                                         width="110px"/></td>
                                <td style="font-size: 9px;text-align: left">${aa.straContext}</td>
                                <td style="font-size: 13px">${aa.praseCount}</td>
                                ...
                                <td>
                                    <c:choose>
                                        <c:when test="${aa.status eq 0 }">
                                            <a href="javascript:changeStatu('${aa.straId }',1,'${aa.straTitle}')"
                                               class="ui blue button">恢复</a></c:when>
                                        <c:when test="${aa.status eq 1 }">
                                            <a href="javascript:window.location.href='${pageContext.request.contextPath }/strategy/straInfo?straId=${aa.straId}'"
                                    ...
                                    <c:if test="${aa.refer eq 0 }">
                                        <a href="javascript:refer('${aa.straId }',1,'${aa.straTitle}')"
...
<script type="text/javascript">
    function changeStatu(straId, statu, straTitle) {
        ...
        var b = window.confirm("确定" + info + "-" + straTitle + "-旅游攻略吗？");
        if (b) {
            $.ajax({
                    type: "post",
                    url: "${pageContext.request.contextPath }/strategy/updateStrategyStatus",
                    data: "straId=" + straId + "&status=" + statu,
    ...
    function refer(straId, statu, straTitle) {
        var b = window.confirm("确定" + "推荐" + straTitle + "这篇攻略吗？");
        if (b) {
            $.ajax({
                type: "post",
                url: "${pageContext.request.contextPath }/strategy/referStrategy",
